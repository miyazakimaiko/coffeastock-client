    ...
    app.get(endpoint + "/user/:userid/recipes-summary", async (req, res, next) => {
      try {
        ...
        const summaryResult = await db.query(`
          SELECT SUM(yield_weight) AS yieldsWeight, 
                 SUM(grounds_weight) AS groundsWeight, 
                 COUNT(*) 
            FROM recipes 
           WHERE user_id = $1`, 
          [req.params.userid]
        ...
        const thirtyDaysSummaryResult = await db.query(`
            SELECT brew_date date, 
                   SUM(grounds_weight) grounds_weight_total,
                   SUM(yield_weight) yield_weight_total,
                   COUNT(*) recipes_count_total
              FROM recipes
             WHERE user_id = $1
          GROUP BY brew_date
        ...
        summaryResult.rows[0].groundWeightTotalsByDay = thirtyDaysSummaryResult.rows.map(row => ({date: row.date, total: row.grounds_weight_total}));
        summaryResult.rows[0].yieldWeightTotalsByDay = thirtyDaysSummaryResult.rows.map(row => ({date: row.date, total: row.yield_weight_total}));
        summaryResult.rows[0].recipesCountsByDay = thirtyDaysSummaryResult.rows.map(row => ({date: row.date, total: row.recipes_count_total}));
